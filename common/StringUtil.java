...
public class StringUtil {
	...
	public static String replace(String s, String sub, String with) {
		int c = 0;
		int i = s.toUpperCase().indexOf(sub.toUpperCase(), c);
		if (i == -1)
			return s;
		...
		synchronized (buf) {
			do {
				...
				buf.append(with);
				c = i + sub.length();
			} while ((i = s.toUpperCase().indexOf(sub.toUpperCase(), c)) != -1);
	...
	public static String replaceHighlight(String s, String sub,
			String textReplace[]) {
		int c = 0;
		int i = s.toUpperCase().indexOf(sub.toUpperCase(), c);
		...
		StringBuffer buf = new StringBuffer(s.length() + sub.length());

		synchronized (buf) {
			do {
				buf.append(s.substring(c, i));
				String strText = s.substring(i, i + sub.length());
				buf.append(textReplace[0] + strText + textReplace[1]);
				c = i + sub.length();
			} while ((i = s.toUpperCase().indexOf(sub.toUpperCase(), c)) != -1);
